//-------------------------------------------------------------------
// $Id: ms_multi.h 675 2012-05-18 12:02:51Z dmitrieva $
//
// Stub declaration of enhanced TMulti class, configuration, and 
// for new phases, TSorpMod and TKinMet implementation functions
// based on the IPM work data structure MULTI that represents chemical
// thermodynamic multisystem work data for GEM IPM-3 algorithm
//
// Copyright (c) 2012 S.Dmytriyeva, D.Kulik, T.Wagner
...
{  // MULTI is base structure to Project (local values)
  ...
  long int
    ...
    FIat,    // max. number of surface site types (old sorption models)
...
double
  ...
  long int
    ...
    *LsMod, // Number of interaction parameters, max. parameter order (cols in IPx),
            // and number of coefficients per parameter in PMc table [FIs][3]
    *LsMdc, //  for multi-site models: [FIs][3] - number of nonid. params per component;
            // number of sublattices nS; number of moieties nM.
//  New: [Fis][6]: + number of DQF coeffs; reciprocal coeffs per end member; reserved
    *IPx,   // Collected indexation table for interaction parameters of non-ideal solutions
            // ->LsMod[k,0] x LsMod[k,1]   over FIs
    // Connection with System/SysEq indexation
...
// Name lists
//  char  (*sMod)[6];   // old: Codes for built-in mixing models of multicomponent phases [FIs]
...
enum {  // Old sorption models - Indexation in a row of the pmp->SATX[][] array
...
// added for TSorpMod implementation
//   long int sizeFIa;     // current size of phSorpMod
//   TSorpMod* (*phSorpMod); // size current FIs -   number of multicomponent phases
//   void Alloc_TSorpMod( long int newFIads );
//   void Free_TSorpMod();
