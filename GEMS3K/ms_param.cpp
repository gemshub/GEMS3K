...
void TMulti::DC_LoadThermodynamicData()
{
  ...
  double Go, Gg, Ge=0., Vv, h0=0., S0 = 0., Cp0= 0., a0 = 0., u0 = 0.;
  double TK, P, PPa;
    ...
    for( j=jb; j<je; j++ )
    {
     ...
     else
     {
      ...
      if( syp->Guns )  // This is used mainly in UnSpace calculations
             Gg = syp->Guns[pmp->muj[j]];    // User-set increment to G0 from project system
      if( syp->GEX && syp->PGEX != S_OFF )   // User-set increment to G0 from project system
             Ge = syp->GEX[pmp->muj[j]];     //now Ge is integrated into pmp->G0 (since 07.03.2008) DK
#else
      if( pmp->tpp_G )
             pmp->tpp_G[j] = Go;
      if( pmp->Guns )
      ...
      Ge = 0.;
#endif
      pmp->G0[j] = Cj_init_calc( Go+Gg+Ge, j, k ); // Inside this function, pmp->YOF[k] can be added!
