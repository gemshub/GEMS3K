...
*
*
*    nx, ny   : Anzahl der Knoten in X- bzw. in Y-Richtung
*    fname    : File-Name
...
/*        printf("a datei nx faktor  %s %d %d %g \n",fname, *nxmax, NCNODEX, faktor );*/
	input = fopen(fname,"r");
/*        printf("b datei nx faktor  %s %d %d %g \n",fname, *nxmax, NCNODEX, faktor );*/
...
int holdat1dalt_(nxmax,fname,hb,text)
...

{
 ...
 printf("nx faktor %d \n",*nxmax);
	input = fopen(fname,"r");
        fscanf(input, "%d %g", &nxx, &faktor );
        printf("nx faktor %d %d %g\n",*nxmax,NCNODEX, faktor );
           for(i=0; i<= (*nxmax)-1; i++)  {
		    /* sum += fread(&h[i][j],sizeof(float),1,input);*/
              sum += fscanf(input," %d",&ihb[i] );
/*        printf("1i= %d j= %d  %d %d \n",i,j,ihb[j][i],sum );*/
