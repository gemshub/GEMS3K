...
int main(int argc, char *argv[]) {
    ...
    std::string inputFile = "../data/input.data";

    reader.outFileDbr = "../data/dbr.dat";
    reader.outFileDhc = "../data/dch.dat";
    reader.outFileIpm = "../data/ipm.dat";

    reader.openInputFile(inputFile, data);
    reader.generateFiles(data);
    std::string form = "CaCO3";
    parser.readFormula(form);

    for (int i = 0; i < parser.atoms.size(); i++){
        std::cout << parser.atoms[i] << "  " << parser.num[i] << std::endl;
    }
