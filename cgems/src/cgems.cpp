...
cGems::cGems(string file) {
   reader.openInputFile(file, data);
   unsigned pos = file.find_last_of("/");
   path = file.substr(0, pos+1);

   reader.outFileDbr = path + "dbr.dat";
   reader.outFileDhc = path + "dch.dat";
   reader.outFileIpm = path + "ipm.dat";
   reader.inFiles    = path + "infile.dat";
   char input_system_file_list_name[256] = "../data/infile.lst";

   reader.generateFiles(data);
   //reader.setupData(data);

   node  = new TNode();
   // (1) Initialization of GEMS3K internal data by reading  files whose names are given in the input_system_file_list_name
   node->GEM_init( input_system_file_list_name );
...
void cGems::showSystem() {
    ...
    for (CompoundStr dcs : data.Compounds) {
        long int x = node->DC_name_to_xCH(dcs.name.c_str());
        long int b = node->DC_xCH_to_xDB(x);
        ...
        stream.width(8);
        stream.precision(3);
        stream << node->Get_nDC(x) << std::endl;
    }
    ...
    for (PhaseStr dcs : data.Phases) {
        long int x = node->Ph_name_to_xCH(dcs.name.c_str());
        long int b = node->Ph_xCH_to_xDB(x);
    ...
    /*
//    long int xCa = cgems.node->IC_xCH_to_xDB("Сa");
//    long int xCa_ion = node->DC_name_to_xCH("Ca+2");
    long int xCal = cgems.node->DC_name_to_xCH("MgCO3");
...
//    long int xbCa_ion = node->DC_xCH_to_xDB(xCa_ion);
    long int xbCal = cgems.node->DC_xCH_to_xDB(xCal);
...
void cGems::run() {
    ...
    {    // Writing results in default DBR file
        ...
        node->GEM_print_ipm( str.c_str() );   // possible debugging printout
    }
    else {
